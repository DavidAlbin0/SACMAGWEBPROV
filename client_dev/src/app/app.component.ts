...
export class AppComponent implements OnInit, DoCheck {
  ...
  confirmarDescargaTodo() {
    // Agregar cuadro de confirmación antes de descargar todo
    const confirmDownloadAll = window.confirm("¿Estás seguro de descargar todos los archivos?");

    if (confirmDownloadAll) {
      window.confirm("¿Estás completamente seguro?");

    } else {
      // Lógica para manejar el caso en que el usuario cancela la descarga
      console.log("Descarga de todos los archivos cancelada por el usuario.");
    }
  }
